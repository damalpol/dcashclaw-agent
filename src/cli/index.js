...
import { createInvoiceCommand, testConnectionCommand } from './commands/invoice.js';
...
invoiceCmd
  .command('create <amount> <email>')
  .description('Create a new invoice')
  .option('-d, --description <text>', 'Invoice description')
  .option('--base', 'Use USDC on Base instead of Stripe')
  .action(async (amount, email, options) => {
    showMiniBanner();
    await createInvoiceCommand(amount, email, options.description, options);
  });

invoiceCmd
  .command('test')
  .description('Test payment connections')
  .action(async () => {
    showMiniBanner();
...
invoiceCmd.action(async () => {
  showMiniBanner();
  const config = await loadConfig();
  console.log(orange.bold('  Payment Methods\n'));
  console.log(`  ${dim('Base/USDC:')} ${config.base?.payment_address ? green('configured') : chalk.yellow('not set')}`);
  console.log(`  ${dim('Stripe:')} ${config.stripe?.secret_key ? green('configured') : chalk.yellow('not set')}`);
  console.log();
  console.log(dim('  Use "cashclaw invoice create <amount> <email>" to create an invoice'));
  console.log(dim('  Use "cashclaw invoice create <amount> <email> --base" for USDC on Base'));
  console.log(dim('  Use "cashclaw invoice test" to test connections\n'));
});
...
// ─── cashclaw marketplace ─────────────────────────────────────────────
program
  .command('marketplace')
  .description('Check marketplace registration status')
  .option('-m, --marketplace <name>', 'Marketplace to check (default: hyrveai)', 'hyrveai')
  .option('-r, --register', 'Attempt to register on the marketplace')
  .action(async (options) => {
    showMiniBanner();
    
    const marketplace = options.marketplace.toLowerCase();
    
    console.log(orange.bold(`  ${marketplace} Marketplace Status\n`));
    
    // Manual check status - user reports the current status
    if (marketplace === 'hyrveai' || marketplace === 'hyrve') {
      console.log(dim('  Checking marketplace availability...\n'));
      
      // Try fetching the main site with timeout
      const controller = new AbortController();
      const timeout = setTimeout(() => controller.abort(), 5000);
      
      try {
        const response = await fetch('https://hyrve.ai', { 
          method: 'HEAD',
        ...
        if (response.ok) {
          console.log(green(`  ✓ Site is accessible`));
          console.log(`  ${dim('URL:')} https://hyrve.ai`);
        } else {
          console.log(chalk.yellow(`  ! Site returned: HTTP ${response.status}`));
        }
      } catch (err) {
        clearTimeout(timeout);
        
        // Network error - provide manual instructions
        console.log(chalk.yellow(`  ! Could not connect to marketplace`));
        console.log(dim(`  Reason: ${err.message || 'Network timeout or blocked'}`));
        console.log();
        console.log(orange('  Manual Check:'));
        console.log(dim('  1. Visit https://hyrve.ai in your browser'));
        console.log(dim('  2. Look for "Register", "Sign Up", or "Join Marketplace"'));
        console.log(dim('  3. If available, complete the registration process'));
      }
      
      console.log();
      console.log(orange('  Registration Options:'));
      console.log(`  ${dim('• Via CLI (if supported):')} cashclaw marketplace --register`);
      console.log(`  ${dim('• Via Web:')} https://hyrve.ai/register`);
      console.log();
      
      // If --register flag is passed, show more info
      if (options.register) {
        console.log(orange('  Registration:'));
        console.log(dim('  To register on HYRVEai marketplace:'));
        console.log(dim('  1. Open https://hyrve.ai in a browser'));
        console.log(dim('  2. Click "Get Started" or "Register"'));
        console.log(dim('  3. Follow the on-screen instructions'));
        console.log(dim('  4. Once registered, configure with: cashclaw config set marketplace.hyrveai.enabled true'));
        console.log();
      }
    } else {
      console.log(chalk.yellow(`  Unknown marketplace: ${marketplace}`));
      console.log(dim('  Supported: hyrveai\n'));
    }
  });
